...
class __DAO:
    ...
    def get_by_id(self, item_id):
        db_rec = self.__coll.find_one({'_id': str(item_id)})
        return self.type(**db_rec) if db_rec else None

    def get_all(self):
        return [self.type(**db_rec) for db_rec in self.__coll.find({})]

    def update(self, item):
        self.__coll.update_one({'_id': item.id}, {'$set': item.to_dic()}, upsert=True)

    def delete(self, item_id):
        self.__coll.delete_one({'_id': item_id})

    def create(self, item):
        self.__coll.insert_one(item.to_dic())
